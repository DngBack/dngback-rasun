...
from trl.trainer.kto_trainer import Any
from trl.trainer.kto_trainer import AutoModelForCausalLM
from trl.trainer.kto_trainer import Callable
from trl.trainer.kto_trainer import concatenate_datasets
from trl.trainer.kto_trainer import contextmanager
from trl.trainer.kto_trainer import create_reference_model
from trl.trainer.kto_trainer import DataCollator
from trl.trainer.kto_trainer import DataLoader
from trl.trainer.kto_trainer import Dataset
from trl.trainer.kto_trainer import deepcopy
from trl.trainer.kto_trainer import defaultdict
from trl.trainer.kto_trainer import Dict
from trl.trainer.kto_trainer import disable_dropout_in_model
from trl.trainer.kto_trainer import DPODataCollatorWithPadding
from trl.trainer.kto_trainer import EvalLoopOutput
from trl.trainer.kto_trainer import F
from trl.trainer.kto_trainer import has_length
from trl.trainer.kto_trainer import inspect
from trl.trainer.kto_trainer import is_peft_available
from trl.trainer.kto_trainer import is_wandb_available
from trl.trainer.kto_trainer import itemgetter
from trl.trainer.kto_trainer import KTOConfig
from trl.trainer.kto_trainer import KTOTrainer
from trl.trainer.kto_trainer import List
from trl.trainer.kto_trainer import Literal
from trl.trainer.kto_trainer import nn
from trl.trainer.kto_trainer import np
from trl.trainer.kto_trainer import nullcontext
from trl.trainer.kto_trainer import Optional
from trl.trainer.kto_trainer import pad_to_length
from trl.trainer.kto_trainer import PartialState
from trl.trainer.kto_trainer import peft_module_casting_to_bf16
from trl.trainer.kto_trainer import PeftModel
from trl.trainer.kto_trainer import prepare_model_for_kbit_training
from trl.trainer.kto_trainer import PreTrainedModel
from trl.trainer.kto_trainer import PreTrainedModelWrapper
from trl.trainer.kto_trainer import PreTrainedTokenizerBase
from trl.trainer.kto_trainer import random
from trl.trainer.kto_trainer import SequentialSampler
from trl.trainer.kto_trainer import torch
from trl.trainer.kto_trainer import tqdm
from trl.trainer.kto_trainer import Trainer
from trl.trainer.kto_trainer import TrainerCallback
from trl.trainer.kto_trainer import TrainingArguments
from trl.trainer.kto_trainer import trl_sanitze_kwargs_for_tagging
from trl.trainer.kto_trainer import Tuple
from trl.trainer.kto_trainer import Union
from trl.trainer.kto_trainer import wandb
from trl.trainer.kto_trainer import warnings
from trl.trainer.kto_trainer import wraps
...
class UnslothKTOConfig(KTOConfig):
    ...
    unsloth_num_chunks: Optional[int] = field(
        default=-1,
        metadata={
            'help': 'Chunk size to reduce memory usage. -1 is most efficient.',
        },
    )
...
class _UnslothKTOTrainer(Trainer):
    ...
    def __init__(
        ...
        if type(args) == TrainingArguments:
            raise ValueError(
                'Please use `KTOConfig` instead TrainingArguments.',
            )
        ...
        with PartialState().local_main_process_first():
            ...
            if eval_dataset is not None:
                ...
                eval_kl_dataset = eval_kl_dataset.map(
                    ...
                    num_proc=args.dataset_num_proc,
                    remove_columns=[
                        c for c in eval_kl_dataset.column_names if c in eval_dataset.column_names
                    ],
            ...
            if len(desirable) != len(undesirable):
                ...
                des_weight_lower_bound = round(
                    (len(undesirable) * self.undesirable_weight / len(desirable)) * 1, 2,
                )
                des_weight_upper_bound = round(
                    (
                        len(undesirable) * self.undesirable_weight /
                        len(desirable)
                    ) * 1.33, 2,
                )
                und_weight_lower_bound = round(
                    (
                        len(desirable) * self.desirable_weight /
                        len(undesirable)
                    ) / 1.33, 2,
                )
                und_weight_upper_bound = round(
                    (len(desirable) * self.desirable_weight / len(undesirable)) / 1, 2,
                )
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if self.precompute_ref_log_probs and not self._precomputed_train_ref_log_probs:
            ...
            for padded_batch in tqdm(iterable=data_loader, desc='Train dataset reference log probs'):
                ...
                reference_completion_logps.append(
                    reference_completion_logp.cpu(),
                )
    ...
    def kto_loss(
        ...
        if policy_rejected_logps.shape[0] != 0 or reference_rejected_logps.shape[0] != 0:
            ...
            rejected_losses = 1 - \
                F.sigmoid(self.beta * (kl - rejected_logratios))
    ...
    def get_batch_loss_metrics(
        ...
        batch = {
            k: (
                v.to(self.accelerator.device) if isinstance(
                    v, torch.Tensor,
                ) else v
        ...
        if all_num_chosen > 0:
            metrics['rewards/chosen_sum'] = self.accelerator.gather(
                chosen_rewards.nansum(),
            ).nansum().item()
            metrics['logps/chosen_sum'] = self.accelerator.gather(
                policy_chosen_logps.nansum(),
            ).nansum().item()
            metrics['count/chosen'] = all_num_chosen

        if all_num_rejected > 0:
            metrics['rewards/rejected_sum'] = self.accelerator.gather(
                rejected_rewards.nansum(),
            ).nansum().item()
            metrics['logps/rejected_sum'] = self.accelerator.gather(
                policy_rejected_logps.nansum(),
            ).nansum().item()
    ...
    def prediction_step(
        ...
        logits = tuple(
            v.unsqueeze(dim=0)
            for k, v in logits_dict.items() if k not in ignore_keys
        )
    ...
    def log(self, logs: Dict[str, float]) -> None:
        ...
        for split in ['chosen', 'rejected']:
            if f'count/{split}' in self._stored_metrics[train_eval]:
                count_sum = torch.Tensor(
                    self._stored_metrics[train_eval][f'count/{split}'],
                ).sum().item()
                logs[f'{train_eval}/rewards/{split}'] = (
                    torch.Tensor(
                        self._stored_metrics[train_eval][f'rewards/{split}_sum'],
                    ).sum().item() / count_sum
                )
                logs[f'{train_eval}/logps/{split}'] = (
                    torch.Tensor(
                        self._stored_metrics[train_eval][f'logps/{split}_sum'],
                    ).sum().item() / count_sum
                )
        ...
        if f'{train_eval}/rewards/chosen' in logs and f'{train_eval}/rewards/rejected' in logs:
            logs[f'{train_eval}/rewards/margins'] = (
                logs[f'{train_eval}/rewards/chosen'] -
                logs[f'{train_eval}/rewards/rejected']
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if os.environ.get('UNSLOTH_FORCE_FLOAT32', '0') == '1':
            ...
            force_float32 = True
        mixed_precision_dtype = os.environ.get(
            'UNSLOTH_MIXED_PRECISION', 'float32',
        )
