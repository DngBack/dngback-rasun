...
from trl.trainer.ppo_trainer import Accelerator
from trl.trainer.ppo_trainer import Adam
from trl.trainer.ppo_trainer import AdaptiveKLController
from trl.trainer.ppo_trainer import BaseTrainer
from trl.trainer.ppo_trainer import Callable
from trl.trainer.ppo_trainer import clip_by_value
from trl.trainer.ppo_trainer import convert_to_scalar
from trl.trainer.ppo_trainer import create_reference_model
from trl.trainer.ppo_trainer import DataCollatorForLanguageModeling
from trl.trainer.ppo_trainer import Dataset
from trl.trainer.ppo_trainer import datasets
from trl.trainer.ppo_trainer import entropy_from_logits
from trl.trainer.ppo_trainer import F
from trl.trainer.ppo_trainer import FixedKLController
from trl.trainer.ppo_trainer import flatten_dict
from trl.trainer.ppo_trainer import gather_object
from trl.trainer.ppo_trainer import inspect
from trl.trainer.ppo_trainer import is_npu_available
from trl.trainer.ppo_trainer import is_torch_greater_2_0
from trl.trainer.ppo_trainer import is_xpu_available
from trl.trainer.ppo_trainer import List
from trl.trainer.ppo_trainer import logprobs_from_logits
from trl.trainer.ppo_trainer import masked_mean
from trl.trainer.ppo_trainer import masked_var
from trl.trainer.ppo_trainer import masked_whiten
from trl.trainer.ppo_trainer import math
from trl.trainer.ppo_trainer import MODEL_CARD_TEMPLATE
from trl.trainer.ppo_trainer import np
from trl.trainer.ppo_trainer import nullcontext
from trl.trainer.ppo_trainer import Optional
from trl.trainer.ppo_trainer import os
from trl.trainer.ppo_trainer import PPOConfig
from trl.trainer.ppo_trainer import PPODecorators
from trl.trainer.ppo_trainer import PPOTrainer
from trl.trainer.ppo_trainer import PreTrainedModelWrapper
from trl.trainer.ppo_trainer import PreTrainedTokenizer
from trl.trainer.ppo_trainer import PreTrainedTokenizerBase
from trl.trainer.ppo_trainer import PreTrainedTokenizerFast
from trl.trainer.ppo_trainer import ProjectConfiguration
from trl.trainer.ppo_trainer import RunningMoments
from trl.trainer.ppo_trainer import set_seed
from trl.trainer.ppo_trainer import stack_dicts
from trl.trainer.ppo_trainer import stats_to_np
from trl.trainer.ppo_trainer import SUPPORTED_ARCHITECTURES
from trl.trainer.ppo_trainer import time
from trl.trainer.ppo_trainer import torch
from trl.trainer.ppo_trainer import typing
from trl.trainer.ppo_trainer import Union
from trl.trainer.ppo_trainer import unwrap_model_for_generation
from trl.trainer.ppo_trainer import version
from trl.trainer.ppo_trainer import WANDB_PADDING
from trl.trainer.ppo_trainer import warnings
from trl.trainer.ppo_trainer import whoami
...
class _UnslothPPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.accelerator.init_trackers(
            ...
            ) if not is_using_tensorboard else config.to_dict(),
            init_kwargs=config.tracker_kwargs,
        )
        self.is_using_text_environment = getattr(
            config, 'use_text_environment', False,
        )
        ...
        if is_deepspeed_used:
            ...
            if not self.is_peft_model and not (
                getattr(
                    self.ref_model.pretrained_model,
    ...
    def step(
        ...
        if self.config.use_score_scaling:
            ...
            score_scaling_factor = self.running.std.to(
                **tensor_to_kwargs,
            ) + torch.finfo(scores.dtype).eps
            if self.config.use_score_norm:
                scores = (
                    scores - self.running.mean.to(
                        **
                        tensor_to_kwargs,
                    )
        ...
        if self.config.score_clip is not None:
            ...
            scores = torch.clip(
                scores.float(), -self.config.score_clip,
    ...
    def prepare_model_inputs(self, queries: torch.Tensor, responses: torch.Tensor):
        if self.is_encoder_decoder:
            input_data = self.data_collator(
                [
                    {
                        'input_ids': q, 'attention_mask': torch.ones_like(
                            q,
                        ),
                    } for q in queries
                ],
            ...
            decoder_inputs = self.data_collator(
                [
                    {
                        'input_ids': r, 'attention_mask': torch.ones_like(
                            r,
                        ),
                    } for r in responses
                ],
            ...
            input_data = self.data_collator(
                [
                    {
                        'input_ids': ids, 'attention_mask': torch.ones_like(
                            ids,
                        ),
    ...
    def compute_advantages(
        ...
        for t in reversed(range(gen_len)):
            ...
            delta = rewards[:, t] + self.config.gamma * \
                nextvalues - values[:, t]
    ...
    def loss(
        ...
        vf_clipfrac = masked_mean(
            torch.gt(vf_losses2, vf_losses1).float(), mask,
        )
        ...
        pg_losses2 = -advantages * \
            torch.clamp(
                ratio, 1.0 - self.config.cliprange,
                1.0 + self.config.cliprange,
            )

        pg_loss = masked_mean(torch.max(pg_losses, pg_losses2), mask)
        pg_clipfrac = masked_mean(
            torch.gt(pg_losses2, pg_losses).float(), mask,
        )
        ...
        return_mean, return_var = masked_mean(
            returns, mask,
        ), masked_var(returns, mask)
        value_mean, value_var = masked_mean(
            values, mask,
        ), masked_var(values, mask)

        stats = dict(
            loss=dict(
                policy=pg_loss.detach(),
                value=vf_loss.detach(), total=loss.detach(),
            ),
            policy=dict(
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        stats['tokens/queries_len_mean'] = torch.mean(
            query_lens,
        ).cpu().numpy().item()
        stats['tokens/queries_len_std'] = torch.std(
            query_lens,
        ).cpu().numpy().item()
        stats['tokens/queries_dist'] = query_lens.cpu().numpy()
        stats['tokens/responses_len_mean'] = torch.mean(
            response_lens,
        ).cpu().numpy().item()
        stats['tokens/responses_len_std'] = torch.std(
            response_lens,
        ).cpu().numpy().item()
        ...
        stats['ppo/val/var_explained'] = 1 - \
            stats['ppo/val/error'] / stats['ppo/returns/var']
        return stats
    ...
    def _show_tokens(self, tokens, masks):
        ...
        for _i, (token, mask) in enumerate(zip(tokens, masks)):
            if mask == 1:
                text.append(
                    self.tokenizer.decode(token.item()),
